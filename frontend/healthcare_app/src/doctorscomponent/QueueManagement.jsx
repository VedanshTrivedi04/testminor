...
// Helper Components (PriorityTag, AppointmentType, StatusBadge) as you provided remain unchanged
...
const QueueManagement = () => {
  ...
  const fetchDoctors = async () => {
    try {
      const response = await apiService.safeRequest('/doctor/');
  ...
  // Actions
  const handlePauseQueue = () => setIsPaused(true);
  const handleResumeQueue = () => setIsPaused(false);
  ...
  // IMPORTANT: Use your new queue API endpoints here

  // Start consultation
  const handleStartConsultation = async (appointment) => {
    try {
      await apiService.startConsultation(appointment.id); // Calls POST /queue/start/{id}/
  ...
  const handleReassign = async () => {
    ...
    try {
      await apiService.safeRequest(`/appointments/${appointment.id}/reschedule/`, {
        method: 'POST',
        body: JSON.stringify({ new_doctor_id: selectedDoctorId }),
      });
  ...
  const handleNoShow = async () => {
    ...
    const appointment = modalState.data;
    try {
      await apiService.skipConsultation(appointment.id); // Calls POST /queue/skip/{id}/
  ...
  const handleAddWalkin = async () => {
    ...
    try {
      const newWalkin = {
        ...
        reason: walkinReason,
        booking_type: 'walkin',
        priority: walkinPriority === 'normal' ? '' : walkinPriority,
      };
      await apiService.createAppointment(newWalkin);
      alert(`Walk-in patient ${walkinName} added successfully.`);
      closeModal();
      await fetchQueueData();
    } catch (err) {
